...
def main():
    ...
    for exp in EXPERIMENTS:
        ...
        if metrics_path.exists():
            rows.append(pd.read_json(metrics_path, typ="series").assign(experiment=exp).to_dict())
        if pred_path.exists():
            pred = pd.read_csv(pred_path)
            cm = confusion_matrix(pred.y_true, pred.y_pred, labels=list(range(10)))
    ...
    if rows:
        pd.DataFrame(rows).to_csv(args.output_dir / "summary_metrics.csv", index=False)
